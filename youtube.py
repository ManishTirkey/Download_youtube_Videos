...
def download(file):
    try:
        ...
        if path is not False:
            ...
            btn_disable()
            d = file.download(path, quiet=True, callback=progressBar)
...
def check_thread(th):
    if th.is_alive():
        ...
        root.after(3000, lambda: downloading_status.config(
            text="Downloading Stop"))
        p_bar['value'] = 0
...
def Change_Qua(delete=None):
    try:
        if delete is not True and stream is not None:
            ...
            c_box2['values'] = list(
                f"{quality}  {size[0]}" for quality, size in object__[extension].items())
            c_box2.current(0)
        else:
...
label = Label(frame1, text="Enter the link",
              font="Castellar 10 bold italic", border=2, relief=SUNKEN)
...
link_entry.grid(row=2, column=0, columnspan=2, sticky=full,
                ipadx=100, ipady=8, padx=5, pady=(20, 0))
...
HD_download_btn = Button(
    frame1, text="HD Audio Video Download", command=HD_Download)
...
def Insert_Info():
    ...
    for info_labels_name, values in information.items():
        info_label = Label(
            frame2, text=f"{info_labels_name}: ", anchor=W, relief=SUNKEN)
        info_label.grid(row=row * 2, column=0, pady=(3, 0),
                        padx=(3, 3), sticky=(E + W), ipadx=100)
